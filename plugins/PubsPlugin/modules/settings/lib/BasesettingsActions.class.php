...
class BasesettingsActions extends sfActions
{
     public function executeIndex(sfWebRequest $request) {
            ...
            if ($user->isAuthenticated()) {
                ...
                $this->forward404Unless($this->user = sfGuardUserPeer::retrieveByPk($this->datos->getId()), sprintf('Object fan does not exist (%s).', $this->datos->getId()));
...
//                            )));
     ...
     public function executeCreate(sfWebRequest $request)
    {
        $user = $this->getUser();
        $this->datos = $user->getGuardUser();
        $this->forward404Unless($u = sfGuardUserPeer::retrieveByPk($this->datos->getId()), sprintf('Object fan does not exist (%s).', $this->datos->getId()));
