...
class pubsAdminGeneratorHelper extends BasePubsAdminGeneratorHelper
{
  ...
  public function linkToComments($object, $params)
  {
    ...
    return '<li class="sf_admin_action_reply">'.link_to(__($params['label'], array(), 'sf_admin'), 'commentAdmin/filter?comment_filters[record_id][text]='.$object->getRecordId().'&comment_filters[record_model][text]='.$object->getRecordModel(),'post=true').' ('.CommentPeer::countComments($object->getRecordId(),$object->getRecordModel()).')</li>';
  }
  
  public function linkToFavlikes($object, $params)
  {
    $comments = FavlikePeer::countFavlikes($object->getRecordId(),$object->getRecordModel());
