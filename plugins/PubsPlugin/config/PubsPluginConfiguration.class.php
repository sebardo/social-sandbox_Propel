...
class PubsPluginConfiguration extends sfPluginConfiguration {
    ...
    public function initialize() {
        ...
        if (in_array('photo', sfConfig::get('sf_enabled_modules', array()))) {
            $this->dispatcher->connect('routing.load_configuration', array('pubsRouting', 'addRouteForPhotos'));
        }

        if (in_array('photoAdmin', sfConfig::get('sf_enabled_modules', array()))) {
            $this->dispatcher->connect('routing.load_configuration', array('pubsRouting', 'addRouteForAdminPhotos'));
        }

        if (in_array('album', sfConfig::get('sf_enabled_modules', array()))) {
            $this->dispatcher->connect('routing.load_configuration', array('pubsRouting', 'addRouteForAlbums'));
        }

        if (in_array('albumAdmin', sfConfig::get('sf_enabled_modules', array()))) {
            $this->dispatcher->connect('routing.load_configuration', array('pubsRouting', 'addRouteForAdminAlbums'));
        }
        /////////////////////////////////////////////////////////////////////////
        /////////////////////////////////////////////////////////////////////////
        if (in_array('event', sfConfig::get('sf_enabled_modules', array()))) {
            $this->dispatcher->connect('routing.load_configuration', array('pubsRouting', 'addRouteForEvents'));
        }

        if (in_array('eventAdmin', sfConfig::get('sf_enabled_modules', array()))) {
            $this->dispatcher->connect('routing.load_configuration', array('pubsRouting', 'addRouteForAdminEvents'));
        }
