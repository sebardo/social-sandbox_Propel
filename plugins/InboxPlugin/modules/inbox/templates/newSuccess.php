...
<?php if ($sf_request->getParameter('user')) { ?>
    <?php $guard = sfGuardUserPeer::retrieveByUsername($sf_request->getParameter('user')); ?>
                var user = '<?php echo $sf_request->getParameter('user') ?>';
                $.get("<?php echo sfConfig::get('app_base_url'); ?>inbox/getUsers?q="+user, function(data){                    
                    for(var key in data){            
                        var r = data[key].split(">");
                    }                       
                    $('#autocomplete_inbox_dest_user_id').val(r[1]);
                    $('#inbox_dest_user_id').val(<?php echo $guard->getId() ?>);
                });
<?php } ?>
